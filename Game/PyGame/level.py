...
from Tiles import Tile, StatictTile, Lucky_block, Coin, Cloud, Tree, Fungus_red, Constraints, Fungus_red_2, End_pipe
...
class Level_1:
        ...
        for fung in self.monsters_sprites:
            if fung.rect.colliderect(player.rect) and not player.on_ground:
                if player.direction.y > 0:
                    ...
                    player.direction.y = 0
                    player.on_ground = True
            if fung.rect.colliderect(player.rect):
                player.rect.move(c.START_POS)
    ...
    def get_fungus_red(self):
        player = self.player.sprite
        for fung in self.fungus_red_sprites:
            if fung.rect.colliderect(player.rect):
                c.COUNT_RED_F += 1
                fung.kill()
    ...
    def check_win(self):
        player = self.player.sprite
        for castle in self.castle_sprites:
    ...
    def run(self):
        ...
        self.trees_sprites.update(c.WORLD_SHIFT)
